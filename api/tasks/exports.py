...
def init(celery_app, **global_decorators):
    ...
    for path in paths:
        if os.path.isdir(path) and '__pycache__' not in path:
            ...
            logger.info(f"[package] package '{package_name}' is valid -> loading the package.")
            if(package_name not in available_packages):
                available_packages.append(package_name)
            else:
                logger.warning(f"[package] there is already a package named {package_name}, package name must be unique -> only the first package named '{package_name}' will be loaded")
